...
A new and improved method is available.  Take a look at [headless WTA training](https://github.com/eraether-portfolio/jit-dynamic-codebooks).  At a high level, what you do is: generate codebooks per minibatch using the transformer hypernetwork (seeded with noise).  This learns a k-means-ish distribution of codes with high codebook utilization.  Then, after roughly ~5k steps you cut over and train on these codes directly.  The only supplemental loss term you need is commitment loss.  The writeup below is kept purely because it is a novel finding I have never seen documented before, and can lead to further discoveries.
